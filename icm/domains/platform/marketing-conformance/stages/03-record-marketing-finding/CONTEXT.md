# Stage 03 — record-marketing-finding

**Job:** record the marketing conformance finding and auto-surface it to the governance
...
1. `[script]` Assemble the marketing conformance finding: overall verdict (conform /
...
3. `[script]` Do **not** correct, re-run, rewrite, re-send, or change config. Any correction
   is `always_gate` to Belle / a human; the deviation routing + closure is A9 (#1467).

## Outputs

`finding.md` — the marketing conformance finding (verdict + diverged rules by reference +
as-of trace id), surfaced to the dashboard. A divergence becomes A9's routing input (routed
to Belle); it is not acted on here.

## Checkpoint — Mark/Belle loop

The finding parks for Mark / Belle. Vera recommends; Belle (or a human) acts. No correction
