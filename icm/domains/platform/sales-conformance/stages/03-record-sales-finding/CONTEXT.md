# Stage 03 — record-sales-finding

**Job:** record the sales conformance finding and auto-surface it to the governance
...
1. `[script]` Assemble the sales conformance finding: overall verdict (conform / diverged),
...
3. `[script]` Do **not** correct, re-run, rewrite, re-send, or change config. Any correction
   is `always_gate` to Chase / a human; the deviation routing + closure is A9 (#1467).

## Outputs

`finding.md` — the sales conformance finding (verdict + diverged rules by reference + as-of
trace id), surfaced to the dashboard. A divergence becomes A9's routing input (routed to
Chase); it is not acted on here.

## Checkpoint — Mark/Chase loop

The finding parks for Mark / Chase. Vera recommends; Chase (or a human) acts. No correction
