...
|---|---|---|---|
| Estate context | `estate-context.md` (stage 01 output) | full | the relationship + service standing + the read asset evidence |
| Lifecycle rubric | `./skills/lifecycle-rubric.md` | all | how to read EOL/aging + the signal-vs-inference discipline |
| Asset estate | silver `cloud_asset` / `device` · `okf:cloud_asset` `okf:device` | this client's CIs | corroborate/refine EOL/age/warranty against the CI rows |
...
1. `[sonnet]` Identify the EOL, aging, and at-risk assets per `lifecycle-rubric.md`. For
   ...
   that produced it (celeste.md guardrail 3). Cite each asset fact to its `cloud_asset`/
   `device` CI row so its provenance stays visible.
...
3. `[script]` Produce the at-risk asset list: each item with its lifecycle status, its
   signals (each cited to its CI / source row), and its client-risk reading. If the estate is
...
lifecycle status, the signals that produced it (measured vs inference labeled, each cited to
its `cloud_asset`/`device` CI row), and its client-risk reading. Security items flagged
advisory-only.

## Audit

- [ ] Every lifecycle verdict labels measured signal vs inference
- [ ] Each asset fact cites its `cloud_asset`/`device` CI row — read-only, never written
- [ ] No asset / age / EOL date invented (only sourced facts)
