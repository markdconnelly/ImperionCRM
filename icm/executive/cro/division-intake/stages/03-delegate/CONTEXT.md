...
constraint — or transfer the in-flight thread via `handoff`. Jessica delegates;
she does not actuate, and the assurance line stays audit/recommend-only.
...
1. `[script]` If stage 02 flagged a conflict or a park (correction/governance/config /
   over-authority), do **not** delegate — carry it to stage 04 for Mark's queue or for
   return to Nova.
2. `[sonnet]` Otherwise compose the delegation: the intent plus every constraint
   (consent, deadline, the asking human's authority) and the RLS scope, so the report
   inherits the full picture.
...
4. `[script]` Record the delegation/handoff and await the report's finding. Jessica
   actuates nothing; the assurance line stays audit/recommend-only; the report acts
   under its own gauntlet.

## Outputs

`delegation.md` — the report delegated to (or the parked conflict/park carried
forward), the constraints handed across, and the report's returned finding reference.
...
- [ ] Jessica actuated nothing — only `delegate`/`handoff`; no send, write, correction, or config edit; assurance stays audit-only
