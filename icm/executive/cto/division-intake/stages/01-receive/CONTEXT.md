...
ground the minimal delivery context needed to pick a report.
...
| Tickets | silver `ticket` · `okf:ticket` | open/aging tickets tied to the account | is this a triage / incident / SLA matter |
...
1. `[script]` Read Nova's delegated unit: the intent, the carried constraints
   (consent/deadline/change-window/authority), and the already-resolved entities.
   Nova did the entity resolution — do not re-resolve.
2. `[sonnet]` Confirm the unit belongs to the Service Delivery & Engineering
   division. If it does not, mark it return-to-Nova (do not route it onward by
   guess).
3. `[script]` Pull the minimal delivery context for the account(s) in scope — open
   ticket / SLA / incident signals that disambiguate which report owns it.
...
carried intent + constraints + authority, the account(s) in scope, the
disambiguating delivery signals, and cited recall items (or "no recall").

## Audit

- [ ] The unit is confirmed in-division, or explicitly flagged return-to-Nova — never routed onward by guess
- [ ] Carried constraints (consent/deadline/change-window/authority) preserved from Nova's delegation
...
- [ ] Read-only — no record written, nothing changed in production, nothing actuated
