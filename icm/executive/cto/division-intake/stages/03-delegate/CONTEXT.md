...
constraint — or transfer the in-flight thread via `handoff`. Dexter delegates; he
does not actuate.
...
1. `[script]` If stage 02 flagged a conflict or a park (production change /
   destructive / identity action / over-authority), do **not** delegate — carry it
   to stage 04 for Luke's queue or for return to Nova.
2. `[sonnet]` Otherwise compose the delegation: the intent plus every constraint
   (consent, deadline, change-window, the asking human's authority) and the RLS
   scope, so the report inherits the full picture.
...
4. `[script]` Record the delegation/handoff and await the report's result. Dexter
...
- [ ] Every constraint from receipt carried (consent/deadline/change-window/authority)
- [ ] Dexter actuated nothing — only `delegate`/`handoff`; no send, write, production change, or identity action
