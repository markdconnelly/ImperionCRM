...
**Advisory Desk (archetype B10)**:
A **read-only consultation** workflow (`archetype: advisory`) that **fields a domain question delegated by the orchestrator (Nova) and returns a persona-shaped, fully-cited answer** — **L0, no actuation, no side effect, no send** (ADR-0138, the tenth Workflow-Doctrine archetype; CONSTITUTION §10). It is how a domain agent becomes **consultable** — it carries the agent's *reasoning* (qualification judgment, deal-risk read), not its guardrails quoted as text. **Division of labour:** Nova answers a plain **retrieval/lookup or cross-domain** question herself; she delegates to the desk only a question needing **domain-expert judgment**; the desk answers **Nova, never the user** (returning to the delegating agent is the §9 `delegate`/`handoff` return path, **not a send**). The read-only contract is **structural** — the conformance gate enforces `autonomy_rung: L0` + `tools ⊆ {pg.read, knowledge.search, memory.recall}` for any advisory workflow — and it binds the Evidence floor (always-cite / cite-or-abstain), never-exceed-caller (§5.2), and Pool-never-bleed. Desks are for the **domain agents** (execs are advisory-by-construction, Nova is the asker); one `<domain>-desk` each, pilot `domains/sales/sales-desk/`.
_Avoid_: confusing with the **Synthesis-brief (B3)** (scheduled, delegate-only, parks a launchpad — the desk is *interactive* Q&A that returns an answer), an advisory desk that actuates/sends (it cannot — it is pure read), a desk for an executive or Nova
