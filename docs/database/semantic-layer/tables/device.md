...
a real silver column (`#1241`/`0193`) with a partial-unique `(account_id, mac)` index over
...
(`firmware_version`, `firmware_updatable`) are now surfaced as silver columns (`0193`), fed
from bronze `unifi_devices` (`0162`).

**Source / precedence column (#1241/0193).** `device` carries a `source` provenance label
...
holds `INSERT, UPDATE` on `device` (`0193`, the 0139/0160 writer-grant pattern) so the on-prem
...
| `mac` | text | network-device natural key (UniFi lateral match, `0193`); partial-unique `(account_id, mac)` over non-null mac |
| `source` | text | contributing-provider label (`website`\|`datto_rmm`\|`unifi`\|`m365`\|`itglue`), `0193`; precedence + replace-from-source scope |
| `os` | text | |
| `status` | text | free-text, e.g. `active` · `retired` |
| `firmware_version` | text | installed firmware (UniFi compliance signal, `0193`) |
| `firmware_updatable` | boolean | available-but-unapplied firmware update (`0193`) |
