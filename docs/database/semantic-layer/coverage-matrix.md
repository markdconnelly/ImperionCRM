...
| [client_offboarding](tables/client_offboarding.md) | Customer Success | B | pii | ✅ | client-offboarding (08-O) — termination → data-return + deprovision header: state machine requested→data_returned→deprovisioned→closed (+cancelled, 08-Q off-ramp); retention/legal-hold clock = deadline-sentinel B9, structural refuse-precondition (no delete under the hold, dial-proof); data-return B7 peel + final-invoice/credit B6 peel (status only — money in invoice/collections, QBO SoR ADR-0123); one OPEN per account; write backend-owed. #1622, epic #1534 |
| [client_offboarding_step](tables/client_offboarding_step.md) | Customer Success | B | pii | ✅ | client-offboarding (08-O) — the deprovision/teardown checklist (onboarding_step mirror in reverse): idempotent per (offboarding, code); owner_seam = A11 cross-owner label (Osiris deprovision, Pierce teardown — Celeste never actuates the delete); always_gate default TRUE; close-on-verification (A9c: fired≠done, verified is). #1622, epic #1534 |
