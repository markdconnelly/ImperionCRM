...
| `/api/recon/actual-counts` | GET | Agreement-reconciliation ACTUAL-count aggregation (#1079, epic #1041) — a per-account roll-up of ACTUALLY-deployed units (`seats` = Σ `license_assignment.quantity`, `devices` = silver `device` count, `backups` = devices with `backup_protected`), read straight from silver. Returns `{ rows, total }` largest-estate-first. The ACTUAL side only; contracted / drift / cost-vs-bill are later slices (#1080–#1082) | Session (middleware-gated) + provisioned acting user, else empty (mirrors `/work/.../activity`); read-only; aggregate counts only (no row-level PII) |
