...
> reads against PostgreSQL). The **agent** and **per-user OAuth connection** surfaces
> are live (below); the remaining external surface (ingestion engines, real sends,
> enrichment) is stubbed (`src/lib/services`) and **fails closed** until it lands —
> document each external endpoint here as it does.
