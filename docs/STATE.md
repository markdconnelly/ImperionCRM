...
- ▶ **Grounding-conflict resolution + domain-owner registry** (ADR-0119, [#1035](https://github.com/markdconnelly/ImperionCRM/issues/1035); agentic-OS contract decision 4): when canon · company · personal disagree at grounding time, serve the most-authoritative *valid* tier **labelled** (anti-stall; `src/lib/grounding/authority.ts`) and bubble the conflict to the **domain owner** to resolve — never hard precedence, never model arbitration (the company-tier twin of `personal_contradiction`). Schema (migration 0176): `domain_owner` registry + `grounding_conflict` workflow (open→resolved/dismissed) + `grounding_conflict_event` ledger + the `app_grounding_conflict_resolver()` resolve predicate. **Tracer**: registry + workflow + interim-answer helper shipped; the resolution **write-back to canon/silver is the deferred follow-up**. Prod-apply Mark-gated.
