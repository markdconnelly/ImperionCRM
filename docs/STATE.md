...
**Schema:** migrations **0001–0164 applied to prod** · **ADRs:** through **0110** · **OKF bundle:**
56 concept files.
...
- `0168` `personal_fact` personal-tier temporal knowledge graph (**#1155, epic #1152, ADR-0114; DORMANT — Mark-gated apply**): owner-private entity–relation–object triples with a Validity Window (`valid_from`/`valid_to`; NULL = live, closed = superseded, never hard-deleted → auditable timeline). Provenance is polymorphic — `(source_kind, source_id)` cites `memory_drawer` (non-agent) or `agent_message` (agent), split by origin (ADR-0113). Owner-axis RLS (ADR-0105, 0153 mechanic). Data layer `src/lib/data/personal-fact.ts` = add/invalidate/timeline/current. Curator god-view + Curated Vault land in #1157; synthesis runtime is BE #302
