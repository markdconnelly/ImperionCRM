...
- `0169` Curated Vault sync substrate + **Personal Curator god-view** (**#1157, epic #1152, ADR-0114 + ADR-0105 amendment; DORMANT — Mark-gated apply**): `personal_vault_file` (blob-path mirror of the per-owner Curated Vault, `sync_state` projected/human_edited/conflict), `personal_curation_event` (append-only god-view ledger), `personal_contradiction` (owner-resolved). God-view = permissive non-BYPASSRLS policy keyed on `current_user = 'imperion-personal-curator'` (Phase-2 INFRA-provisioned role) over ALL personal surfaces incl. existing `memory_drawer`/`personal_fact`; LP gets SELECT-only god-view on `memory_drawer` (#300). Owner-facing data layers `personal-vault-file.ts`/`personal-contradiction.ts`/`personal-curation-event.ts`. Curator runtime = BE #302
