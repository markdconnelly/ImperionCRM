...
| 12 | **[The agent roster](agent-roster.md)** | The staffed org chart: the eight named agents (Felix · Chase · Belle · Audrey · Vance · Pierce · Celeste · Vera), one per workspace, with persona + voice + guardrails, and the rule that **each agent's guardrails are the source of truth for its governance config.** Felix (Service) is the wedge, built first. |
| 13 | **[The native approval cockpit](approval-cockpit.md)** | The cross-agent supervision surface: the `/operator/cockpit` queue of EVERY agent's actions parked above its autonomy ceiling — proposing agent · tier · rationale · target · dial decision — approve / edit / reject wired to the backend decide endpoint (ADR-0107 D5, backend #267). The agent-agnostic twin of the Technician-only cockpit (#11). |
| 14 | **[The org visualization](org-visualization.md)** | The `/org` surface: the agent org tree (Nova → C-suite → domain agents → playbooks) rendered with react-flow from the single SoT (`icm/org.yaml` + `icm/**`, generated into `src/data/org-graph.json`), plus a click-node side panel (persona · ceiling · tool/room budget · live dial · queue) — the structure browser and the live overlay on one canvas (#1539, ADR #1535). |
