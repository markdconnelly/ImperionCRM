...
| Chief Information Security Officer | officer | MSP-under-continuous-attack threat model, grounded in posture gold data | — |
...
Cap: ≤7 seated per session (5 officers + ≤2 invited advisors). Advisors are counsel, not
votes. The **human CISO (Mark)** reviews every concluded recommendation and ratifies or
overrules it with a written rationale (`board_recommendation.review_status`).
...
```mermaid
flowchart TD
    A[Convene: topic + context + advisor invites] --> B{Budget OK?}
    B -- no --> P[paused — no spend]
    B -- yes --> C[Compose board packet — cheap tier, persisted]
    C --> D[Round 1: opening positions — officers + invited advisors]
    D --> E[Round 2: cross-examination]
    E --> F[Facilitator synthesis — Lencioni character, disagreements surfaced]
